...
export class YogaService {
    ...
    subirFotos(element: HTMLDocument): string[] {
        ...
        for (let selectedFile of [(<HTMLInputElement>element.getElementById('file')).files[0]]) {
            ...
            var date = new Date();
            var hor = date.getDate() + " " + date.getMilliseconds()+ " " +date.getTime();
            let folder = 'imagenes'
    ...
    subirFotosCarousel(element: HTMLDocument): string[] {
        ...
        for (let selectedFile of [(<HTMLInputElement>element.getElementById('file3')).files[0]]) {
            console.log(selectedFile);
            // Make local copies of services because "this" will be clobbered
            var date = new Date();
            var hor = date.getDate() + " " + date.getMilliseconds()+ " " +date.getTime();
    ...
    subirFotosCarousel_3(element: HTMLDocument): string[] {
        ...
        for (let selectedFile of [(<HTMLInputElement>element.getElementById('file4')).files[0]]) {
            console.log(selectedFile);
            // Make local copies of services because "this" will be clobbered
            var date = new Date();
            var hor = date.getDate() + " " + date.getMilliseconds()+ " " +date.getTime();
    ...
    subirFotosInstructores(element: HTMLDocument, coment: string,name:string): string[] {
        this.int = new Inst('', '','');
        let selectedFile = (<HTMLInputElement>element.getElementById('file2')).files[0]
        console.log(selectedFile);
        // Make local copies of services because "this" will be clobbered
        var date = new Date();
        var hor = date.getDate() + " " + date.getMilliseconds()+ " " +date.getTime();
